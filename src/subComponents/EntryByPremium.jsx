...
import { updateParameter } from "../slices/PremiumEntrySlice";
...
const EntryByPremium = () => {
  const { StrikeParameter } = useSelector((store) => store.PremiumEntry);
  const dispatch = useDispatch();
  const handlePremiumChange = (e) => {
    const { value } = e.target;
    dispatch(updateParameter(value));
  };
  const incrementPremium = () => {
    dispatch(updateParameter(StrikeParameter + 1));
  };
  const decrementPremium = () => {
    if (StrikeParameter > 0) dispatch(updateParameter(StrikeParameter - 1));
  };
