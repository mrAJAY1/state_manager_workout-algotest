...
import { updateLower, updateUpper } from "../slices/PremiumRangeSlice";
...
`;
const EntryByPremiumRange = ({ values, setValues }) => {
  const { StrikeParameter } = useSelector((store) => store.PremiumRange);
  const dispatch = useDispatch();
  const handleUpperParamChange = (e) => {
    const { value } = e.target;
    dispatch(updateUpper(value));
  };
  const handleLowerParamChange = (e) => {
    const { value } = e.target;
    if (value <= StrikeParameter.Upper) dispatch(updateLower(value));
  };

  const incrementUpperParam = () => {
    let value =
      StrikeParameter.Upper === "" ? 0 : parseInt(StrikeParameter.Upper);
    dispatch(updateUpper(value + 1));
  };

  const decrementUpperParam = () => {
    let value =
      StrikeParameter.Upper === ""
        ? StrikeParameter.Lower
        : StrikeParameter.Upper;

    if (value > StrikeParameter.Lower) dispatch(updateUpper(value - 1));
  };
  const incrementLowerParam = () => {
    let value = StrikeParameter.Lower === "" ? 0 : StrikeParameter.Lower;

    if (value < parseInt(StrikeParameter.Upper))
      dispatch(updateLower(value + 1));
  };

  const decrementLowerParam = () => {
    let value = StrikeParameter.Lower === "" ? 0 : StrikeParameter.Lower;
    if (value > 0) dispatch(updateLower(value - 1));
  };

  return (
    <>
      <NumberContainer>
        <label htmlFor="">Lower Range</label>
        <div className="number_input_container">
          <input
            name="Lower"
            value={StrikeParameter.Lower}
            onChange={handleLowerParamChange}
            type="number"
            min="1"
          />
          <svg
            className="icon_increment"
            onClick={incrementLowerParam}
            width="8"
            height="5"
            viewBox="0 0 8 5"
            fill="none"
    ...
    1.11259 7.97187 0.974248C7.91441 0.835908 7.7782 0.747228 7.6285 0.750066Z"
      ...
      <NumberContainer>
        <label htmlFor="">Upper Range</label>
        <div className="number_input_container">
          <input
            name="Lots"
            value={StrikeParameter.Upper}
            onChange={handleUpperParamChange}
            type="number"
            min="1"
          />
          <svg
            className="icon_increment"
            onClick={incrementUpperParam}
            width="8"
            height="5"
            viewBox="0 0 8 5"
            fill="none"
...
export default EntryByPremiumRange;
