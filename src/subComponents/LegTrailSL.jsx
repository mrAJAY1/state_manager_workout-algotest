...
import { updateLegTrailSL } from "../slices/NewLegComponentSlice";
...
const LegTrailSL = ({ dispatch, index, LegTrailSLValue }) => {
  const [trailState, setTrailState] = useState(false);
  const [values, setValues] = useState({ ...LegTrailSLValue });

  const trailClicked = () => {
    setTrailState(!trailState);
  };
  const typeChange = (e) => {
    const { value } = e.target;
    const Instr = values.Value.InstrumentMove || 1;
    const StopLoss = values.Value.StopLossMove || 1;
    setValues({
      Value: { InstrumentMove: Instr, StopLossMove: StopLoss },
      Type: value,
    });
  };
  const updateSL = (e) => {
    setValues({
      ...values,
      Value: { ...values.Value, StopLossMove: e.target.value },
    });
  };
  const updateInstrument = (e) => {
    setValues({
      ...values,
      Value: { ...values.Value, InstrumentMove: e.target.value },
    });
  };
  const incrementInstrument = () => {
    const num = values.Value.InstrumentMove
      ? values.Value.InstrumentMove
      : values.Value.InstrumentMove === ""
      ? 0
      : 0;
    setValues({
      ...values,
      Value: { ...values.Value, InstrumentMove: num + 1 },
    });
  };

  const decrementInstrument = () => {
    values.Value.InstrumentMove > 0 &&
      setValues({
        ...values,
        Value: {
          ...values.Value,
          InstrumentMove: values.Value.InstrumentMove - 1,
        },
      });
  };
  const incrementSL = () => {
    const num = values.Value.StopLossMove
      ? values.Value.StopLossMove
      : values.Value.StopLossMove === ""
      ? 0
      : 0;
    ...
    setValues({
      ...values,
      Value: { ...values.Value, StopLossMove: num + 1 },
    });
  };
  const decrementSL = () => {
    values.Value.StopLossMove > 0 &&
      setValues({
        ...values,
        Value: { ...values.Value, StopLossMove: values.Value.StopLossMove - 1 },
      });
  };
  ...
  useEffect(() => {
    ref.current += 1;
    !trailState && (ref.current = 0);
    if (ref.current === 1) {
      setValues({
        Type: "TrailStopLossType.Points",
        Value: { InstrumentMove: 1, StopLossMove: 1 },
      });
      dispatch(
        updateLegTrailSL({
          value: values,
    ...
    trailState
      ...
      : dispatch(
          updateLegTrailSL({
            value: {
              Type: "none",
              Value: { InstrumentMove: null, StopLossMove: null },
            },
            index,
          })
        );
  ...
  return (
    <OptionalInputContainer>
      <div className="optionalInputContainer">
        <div className="checkboxContianer" onClick={trailClicked}>
          <label>
            <span className={trailState ? "checked" : null}></span>
            Trail SL
            <svg
              width="14"
              height="14"
        ...
        <OptionalInput
          style={{
            opacity: !trailState && "0.3",
            pointerEvents: !trailState && "none",
          }}
        >
          <MomentumSelect>
            <span>
              <select value={values.Type} onChange={typeChange}>
          ...
          <NumberContainer className="numberContainer">
            <NumberInput>
              <input
                min={1}
                value={values.Value.InstrumentMove || 1}
                onChange={updateInstrument}
                type="number"
              />
              <svg
                onClick={incrementInstrument}
          ...
          <NumberContainer className="numberContainer">
            <NumberInput>
              <input
                min={1}
                type="number"
                value={values.Value.StopLossMove || 1}
                onChange={updateSL}
              />
              <svg
                className="numberIncrement"
